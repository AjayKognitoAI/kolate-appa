...
docker run -d --name ${projectName} -p 9000:9000 --restart unless-stopped --network kolatenw -e SPRING_PROFILES_ACTIVE=prod -e DOCKER_ENV=prod  kolate/${projectName}:latest
