...
docker run -d --name ${projectName} -p 9010:9010 --restart unless-stopped --network kolatenw -e SPRING_PROFILES_ACTIVE=test -e DOCKER_ENV=test  kolate/${projectName}:latest
