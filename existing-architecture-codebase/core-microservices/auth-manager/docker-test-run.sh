...
docker run -d --name ${projectName} -p 9005:9005 --restart unless-stopped --network kolatenw -e SPRING_PROFILES_ACTIVE=test -e DOCKER_ENV=test  kolate/${projectName}:latest
