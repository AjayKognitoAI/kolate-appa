...
docker run -it --name "${projectName}" -p 9025:9025 --restart unless-stopped --network kolatenw -e SPRING_PROFILES_ACTIVE=dev -e DOCKER_ENV=dev  kolate/"${projectName}":latest
