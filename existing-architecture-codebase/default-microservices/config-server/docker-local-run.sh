...
docker run -d --name ${projectName} -p 8888:8888 --restart unless-stopped --network kolatenw -e SPRING_PROFILES_ACTIVE=dev -e DOCKER_ENV=dev  kolate/${projectName}:latest
