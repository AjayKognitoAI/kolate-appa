...
docker run -d --name ${projectName} -p 9015:9015 --restart unless-stopped --network kolatenw -e SPRING_PROFILES_ACTIVE=dev -e DOCKER_ENV=dev -e SERVICE_NAME=${projectName} kolate/${projectName}:latest
