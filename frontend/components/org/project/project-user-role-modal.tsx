...
interface MemberRoleModalProps {
  open: boolean;
  onClose: () => void;
  onConfirm: (newRole: any) => void;
  member?: any;
  loading?: boolean;
}

const ProjectUserRoleModal: React.FC<MemberRoleModalProps> = ({
  open,
  onClose,
  onConfirm,
  member,
  ...
  const handleConfirm = async () => {
    if (selectedRole && member && member.id) {
      setError(null);
      setSuccess(null);
      try {
        // Here we assume the member's id is the project id for this context
        ...
        const role = roleOptions.find((ele) => ele?.value === selectedRole);
        onConfirm({ role_name: role?.label, role_id: selectedRole });
        onClose();
      } catch (error: any) {
        toast.error(
    ...
    >
      ...
      <DialogContent sx={{ pt: 3, pb: 2 }}>
        ...
        <Typography variant="body2" sx={{ mb: 2, color: "text.secondary" }}>
          ...
          and access levels across the project.
        </Typography>
...
export default ProjectUserRoleModal;
